        ...
        var SocketbaseFactory = function (url, paramDefaults, actions, options) {
            ...
            SocketResource.get = function () {
                ...
                getter.$promise.then(function (resource) {
                    // re attach update and delete to object         
                    resource.$update = function (parameters, callback) {
                        var data = this;
            ...
            SocketResource.query = function (parameters) {
                ...
                var doQuery = function (args) {
                    return SocketResource.resource.query.apply(SocketResource.resource, args);
                };
                ...
                query.$promise.then(function (results) {
                    ...
                    Socket.on('save-' + module, function (newContent) {
                        if (meetsRequirements(newContent, parameters)) {
                            var index = results.map(function (result) {
                                return result[idField];
                            }).indexOf(newContent[idField]);
                            if (index == -1) {
                                ...
                                results.downloadedUpdates.push({
                                    type: 'save'
                                    , time: new Date()
                                    , content: newSocketResource
                                });
                            }
                        }
                    });
